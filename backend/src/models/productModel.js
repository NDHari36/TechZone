...
class Product {
  static async findAll({
    ...
    let sql = `
      SELECT 
          p.id, p.name, p.category_id, b.name AS brand_name, c.name AS category_name,
          ...
          (SELECT SUM(i.quantity) FROM inventories i JOIN product_variants v ON i.variant_id = v.id WHERE v.product_id = p.id) AS total_stock
      FROM products p
    ...
    sql += ` GROUP BY p.id, p.name, p.category_id, b.name, c.name ORDER BY p.created_at DESC LIMIT ? OFFSET ? `;
  ...
  static async getSuggestions(keyword) {
    const sql = `SELECT id, name FROM products WHERE name LIKE ? AND is_active = 1 LIMIT 5`;
  ...
  static async getDetail(id) {
    try {
      ...
      const imagesSql = `SELECT id, image_url, is_primary FROM product_images WHERE product_id = ? ORDER BY sort_order ASC`;
      const [images] = await db.query(imagesSql, [id]);

      const variantsSql = `
        SELECT pv.*, i.quantity, i.safety_stock 
      ...
      const specsSql = `
        SELECT variant_id, group_name, name, value_text, unit 
      ...
      const variantsWithSpecs = variants.map((variant) => {
        return {
          ...variant,
          product_variant_specs: allSpecs.filter(
            (spec) => spec.variant_id === variant.id,
          ),
        };
      });
  ...
  static async create(productData) {
    ...
    try {
      ...
      const [resProduct] = await connection.query(
        `INSERT INTO products (brand_id, category_id, name, description) VALUES (?, ?, ?, ?)`,
      ...
      if (variants && variants.length > 0) {
        for (const variant of variants) {
          const [resVariant] = await connection.query(
            `INSERT INTO product_variants (product_id, sku, ram, storage, color, price) VALUES (?, ?, ?, ?, ?, ?)`,
            [
              productId,
              variant.sku,
              variant.ram || null,
          ...
          if (variantSpecs.length > 0) {
            ...
            await connection.query(
              `INSERT INTO product_variant_specs (variant_id, group_name, name, value_text, unit, is_filterable, sort_order) VALUES ?`,
              [specValues],
            );
          }
        }
  ...
  static async update(id, productData) {
    ...
    try {
      ...
      await connection.query(
        `UPDATE products SET brand_id = ?, category_id = ?, name = ?, description = ?, is_active = ? WHERE id = ?`,
        [
          brand_id,
          category_id,
          name,
      ...
      if (variants && variants.length > 0) {
        ...
        for (const variant of variants) {
          ...
          if (variantId) {
            await connection.query(
              `UPDATE product_variants SET sku = ?, ram = ?, storage = ?, color = ?, price = ? WHERE id = ?`,
              [
                variant.sku,
                variant.ram || null,
                variant.storage || null,
          ...
          await connection.query(
            `DELETE FROM product_variant_specs WHERE variant_id = ?`,
            [variantId],
          );
  ...
  static async getReviews(productId) {
    ...
    SELECT 
      ...
      u.username AS display_name, u.full_name,
