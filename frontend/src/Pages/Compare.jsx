...
function Compare() {
  ...
  productsData.forEach((prod) => {
    ...
    specs.forEach((spec) => {
      ...
      if (!globalGroups[matchedGroup]) {
        globalGroups[matchedGroup] = new Set();
      }
  ...
  const getQuickCompareData = (prod) => {
    ...
    if (variant.ram) {
      bullets.push(`RAM: ${variant.ram}`);
    }

    if (variant.storage) {
      bullets.push(`Dung lượng: ${variant.storage}`);
    }

    const camSau = findSpec(["camera sau"]);

    if (camSau) {
      bullets.push(`Camera sau: ${camSau}`);
    }

    const camTruoc = findSpec(["camera trước"]);

    if (camTruoc) {
      bullets.push(`Camera trước: ${camTruoc}`);
    }
  ...
  return (
    <>
          ...
          <div className="border-t border-l border-gray-200 rounded-tl-xl overflow-hidden shadow-sm">
            ...
            <div className="flex bg-white">
              ...
              {[...Array(3 - productsData.length)].map((_, idx) => (
                ...
                >
                  <div className="w-16 h-16 border-2 border-dashed border-gray-300 group-hover:border-blue-300 rounded-full flex items-center justify-center mb-3">
                    <Plus size={24} />
                  </div>
      ...
      {isSearchOpen && (
        <div className="fixed inset-0 bg-black/60 z-[999] flex items-start justify-center pt-[8vh] px-4">
              ...
              >
                <div className="relative group">
                  ...
                  <input
                    ...
                    className="w-full pl-14 pr-32 py-5 bg-white border border-slate-200 rounded-[2rem] shadow-sm focus:outline-none focus:ring-4 focus:ring-slate-100"
                  />

                  <div className="absolute inset-y-2 right-2 flex items-center">
                    <button
                      type="submit"
                      className="h-full px-8 bg-slate-900 text-white text-sm font-black uppercase tracking-widest rounded-[1.5rem]"
                    >
                      Tìm kiếm
                    </button>
                  </div>
